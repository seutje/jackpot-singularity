...
const App: React.FC = () => {
  ...
  const spawnCoin = useCallback((type: CoinType, isFree: boolean = false) => {
     // Check for Extender upgrade to widen drop range
     // We need to access state, so we use a functional update for setGameState usually,
     // but here we need values for position calculation.
     // For simplicity in this logic block, we assume `gameState` in scope is fresh enough 
     // or we calculate solely based on active artifacts if we passed them differently.
     // However, inside a useCallback without `gameState` dep, `gameState` is stale.
     // To fix this, we'll read the Extender level inside the setGameState or pass it.
     
     // Better approach: Calculate position logic inside the state update or use a ref for artifacts.
     // Since this is a simple game, we will update deck state first, then spawn object.
     
     setGameState(prev => {
        // Validation: If not free, must have deck count
        if (!isFree && prev.deck[type] <= 0) return prev;

        // Decrement deck if not free
        const newDeck = isFree ? prev.deck : { ...prev.deck, [type]: prev.deck[type] - 1 };

        // Calculate Position
        const extender = prev.artifacts.find(a => a.id === 'extender');
        ...
        // We need to update activeCoins here, but we are inside setGameState.
        // We can trigger a side effect or simply set activeCoins outside.
        // To avoid double-render issues, we'll set activeCoins in a setTimeout or use a separate handler.
        // Let's use the external setActiveCoins.
        setTimeout(() => {
  ...
  }, []); // Intentionally empty deps to avoid recreating, using state setter callback for logic
